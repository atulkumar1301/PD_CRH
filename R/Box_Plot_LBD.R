...
df_2 <- fread ("/Volumes/ATUL_6TB/Work/Projects/Nicholas_Ashton/Data_SAA-AD_FTD_VaD_vs_SAA+_LBD.txt")
par(cex.axis=1.5)
boxplot(CRH__Oncology_o2csf__NPX ~ Group, data = df_2, lwd = 2, ylab = 'CSF CRH', notch = TRUE, cex.lab = 1.5, main = "C) SAA- AD/FTD/VaD vs SAA+ LBD", cex.main = 1.5)
stripchart(CRH__Oncology_o2csf__NPX ~ Group, vertical = TRUE, data = df_2, 
           method = "jitter", add = TRUE, pch = 20, col = c('#E69F00', '#CC79A7'), cex = 2, jitter = 0.3)


df_3 <- fread ("/Volumes/ATUL_6TB/Work/Projects/Nicholas_Ashton/Data_SAA-Ctrl_vs_SAA+Ctrl.txt")
par(cex.axis=1.5)
boxplot(CRH__Oncology_o2csf__NPX ~ Group, data = df_3, lwd = 2, ylab = 'CSF CRH', notch = TRUE, cex.lab = 1.5, main = "D) SAA- CTR vs SAA+ CTR", cex.main = 1.5, col.main = "#0072B2")
